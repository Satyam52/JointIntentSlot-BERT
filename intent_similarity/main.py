...
def main(args):
    ...
    intent_label_token = [tokenizer(seq.replace(
        "_", " "), padding='max_length', max_length=50, truncation=True) for seq in intent_idx2word.values()]
    ...
    model = IntentClassification.from_pretrained(
        "bert-base-uncased", config=model_config, num_intent_labels=len(intent_labels),  intent_labels_token=intent_label_token)
    model.to('cuda')
...
def evalFun(path, args):
    ...
    model = IntentClassification.from_pretrained(path, num_intent_labels=len(intent_labels), intent_labels_token=intent_label_token)
    model.init_intent_desc()
    ...
    def predict(model, seqs):
        ...
        for i in tqdm.tqdm(range(len(seqs))):
            ...
            model.eval()
            with torch.no_grad():
                _, (intent_logits) = model(**input_seq, eval=True)
