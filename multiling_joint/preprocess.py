...
def createSlotVocab(annot_text):
    vocab = list()
    arr = re.split(r"[\[\]]", annot_text)
    for wordGroup in arr:
        if ":" in wordGroup:
            namedEntity, _ = wordGroup.replace("]", "").split(":")
            vocab.append('B-' + namedEntity)
            vocab.append('I-' + namedEntity)
    return vocab
...
if __name__ == "__main__":
    ...
    with open(f"data/processed/intent_label_vocab", "w", encoding="utf-8") as file:
        file.writelines("\n".join(data["intent"].unique()))
        file.close()

    with open(f"data/processed/domain_label_vocab", "w", encoding="utf-8") as file:
        file.writelines("\n".join(data["scenario"].unique()))
        file.close()

    with open(f"data/processed/slot_label_vocab", "w", encoding="utf-8") as file:
        slots = data["annot_utt"].apply(createSlotVocab).values
        unique_slots = list({word.strip() for x in slots for word in x})
        unique_slots.append("O")
        file.writelines("\n".join(unique_slots))
        file.close()
